# function_app.py — versie v12.1 (jouw v12 + CORS + sterkere output-extract)
# - Proxy naar Prompt Flow
# - JSON + multipart (PDF/DOCX)
# - Mode: QUICKSCAN/REVIEW/DRAFT (heuristisch als niet meegegeven)
# - CORS af, én robuuster chat_output uit PF-response
...
# ---------------- CORS (enkel whitelist exact origins) ----------------
ALLOWED_ORIGINS = set([
    "https://nice-bay-0e1280e03.2.azurestaticapps.net",  # jouw SWA origin
...
def extract_chat_output_from_json(pf_data: dict) -> str:
    ...
    - Ondersteunt zowel top-level velden als outputs.chat_output (veel PF deployments).
    """
    if pf_data is None:
        return ""

    # 1) outputs.chat_output
    try:
        v = pf_data.get("outputs", {}).get("chat_output")
        if isinstance(v, str) and v.strip():
            return v.strip()
    except Exception:
        pass

    # 2) Bekende top-level varianten
...
def _normalize_mode(value: str | None) -> str | None:
    if not value:
        return None
    v = value.strip().upper()
    # synoniemen die uit de UI of typend kunnen komen
...
def _infer_mode_from_text(question: str) -> str:
    """
    Eenvoudige heuristiek wanneer 'mode' niet expliciet is meegestuurd.
    """
    q = (question or "").lower()
    ...
    try:
        ...
        if "multipart/form-data" in content_type:
            ...
            try:
                ...
                return func.HttpResponse(
                    "Ongeldig multipart-verzoek (decode fout).",
                    status_code=400,
                    mimetype="text/plain",
                    headers=_cors_headers(req)
                )
            ...
            # mode uit formveld (we ondersteunen 'mode' en 'tm_mode')
            ...
            mode = _normalize_mode(body.get("mode") or body.get("tm_mode"))
        ...
        try:
            ...
            if not chat_output:
                chat_output = json.dumps(pf_data, ensure_ascii=False)[:4000]
            return func.HttpResponse(
                body=json.dumps({"chat_output": chat_output}, ensure_ascii=False),
                status_code=200,
            ...
            return func.HttpResponse(
                body=json.dumps({"chat_output": txt[:4000]}, ensure_ascii=False),
                status_code=200,
                mimetype="application/json",
                headers=_cors_headers(req)
            )
        ...
        if debug_flag:
            return func.HttpResponse(
                json.dumps({"error": body_text}, ensure_ascii=False),
                status_code=200,
                mimetype="application/json",
                headers=_cors_headers(req)
            )
